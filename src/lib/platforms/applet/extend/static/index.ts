import { AxiosInstance, AxiosRequestConfig, AxiosStatic, CancelStatic, CancelTokenStatic,AxiosResponse} from '../../../../request-config';
...
function compatible(config:AxiosRequestConfig={},baseConfig:AxiosRequestConfig):Record<string, any> {
    ...
    applaetInstance.upload =function<T> (config:AxiosRequestConfig):Promise<AxiosResponse<T>> {
        return new Promise(function (resolve, reject) {
            ...
            let cancel = wx.uploadFile({
                ...
                header:resultConfig.headers,
                url:control.getURL(resultConfig),
                success:function (data:Record<string, any>){
                    return resolve({
                        status:data.statusCode,
                        // @ts-ignore
                        data:data.data,
