...
export default class UploadExtend extends CacheDataExtend{
    ...
    constructor(cache:Cache,private cacheParams?:RequestUploadCache){
        super(UploadExtend.getCacheParams(cacheParams),'noUpdate',cache);
    }

    static getCacheParams(cache:RequestUploadCache){
        if(cache) {
            return {
                ...cache,
                option:{
    ...
    transfSpeed(data:RequestContxtParams):UploadExtendSpeedParams{
        ...
        let speedParams:UploadExtendSpeedParams = UploadExtend.getSpeedParams({
    ...
    static getSpeedParams(data:{loaded:number,total:number,analysis?:boolean,mergeAnalysis?:boolean},contxtParams:Record<string,any> = {}):UploadExtendSpeedParams{
