...
export default class SessionLocal extends Memory {
    ...
    setItem<T>(key: string, value: T,option:StorageSetOption<T>) {
        ...
        if (this.timeObject) platforms.platform.clearTimeout && platforms.platform.clearTimeout(this.timeObject[key]);

        if (!this.timeObject) this.timeObject = {};
        this.timeObject[key] = platforms.platform.setTimeout && platforms.platform.setTimeout( ()=> {
    ...
    updateStorageOption(){
        (this.setOptionTime);
        this.setOptionTime = platforms.platform.setTimeout && platforms.platform.setTimeout(()=>{

            if (this.updateExpire()) return platforms.platform.clearTimeout && platforms.platform.clearTimeout(this.setOptionTime);
