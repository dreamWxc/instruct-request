...
export default class CacheControl implements CacheControlClass {
    ...
    addRollBack<T extends keyof CacheExtendParams = keyof CacheExtendParams>(key?:T,value?:CacheExtendParams[T]):this{
        ...
        if (key === null ? true : this.imageOptions[key] !== value) {
            ...
            // 清楚异步执行
            platforms.platform.clearTimeout && platforms.platform.clearTimeout(this.asyncOptionUpdateTime);
            // 设置当前正在进行异步操作
            this.optionUpdate = true;
            // 获取异步执行
            this.asyncOptionUpdateTime = platforms.platform.setTimeout(()=> this.completeRollBack());
        }
