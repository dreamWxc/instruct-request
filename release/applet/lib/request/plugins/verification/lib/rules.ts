import { VerificationTrigger } from './type';
...
const verificationRules:VerificationTrigger={

    // 鉴定length
    length(option){
        if(option.value) {
            if(option.value.hasOwnProperty('length')) {
                return (option.value as {length:number}).length === option.option.value;
            } else {
    ...
    empty(option){
        ...
        if(type === 'object' || type === 'string') {
            ...
            return type as unknown as boolean;
        } else if(type ==='number'){
            return value > 0;
        }
