...
export default class UploadSlice extends UploadExtend {
    request;
    ...
    // 是否有可以执行
    static jurisdiction = !!(platforms.platform.FileReader && platforms.platform.Blob);
    // @ts-ignore
    blobSlice = File.prototype.slice || File.prototype.mozSlice || File.prototype.webkitSlice;
    // 触发upload
    triggerUpload(config, requestConfig, option, uploadContxt) {
        if (!uploadContxt || !uploadContxt.storageContent || !uploadContxt.unique) {
            // @ts-ignore
            let fileOption = Object.assign({}, option, requestConfig.file);
            let resultUploadContxt = uploadContxt || fileOption.controller;
            if (resultUploadContxt && resultUploadContxt.__unique)
                return;
            let uploadConfig = this.createConfig(fileOption);
    ...
    // 触发
    triggerUnique({ hash, config, requestConfig, uploadConfig, fileOption, uploadContxt }) {
        ...
        if (!uploadContxt || !uploadContxt.storageContent || !uploadContxt.surplus) {
            ...
            if (fileOption.cache && fileOption.record) {
                ...
                if (resutlStorageContxt && resutlStorageContxt.data) {
                    storageContxt = resutlStorageContxt.data;
                }
            }
            // 创建基本配置
            let contxt = {
            ...
            if (storageContxt) {
                ...
                let surplus = [];
                for (let i = uploadConfig.total - 1; i >= 0; i--) {
                    surplus.push(i);
                }
                contxt.surplus = surplus;
                if (uploadContxt) {
        ...
        switch (fileOption.mode) {
            case 'all':
                number = uploadConfig.total;
                break;
            case 'many':
                number = fileOption.manyNumber || 1;
                break;
            default:
                number = 1;
                break;
        ...
        const callback = (response) => {
            // 如果为暂停 停止执行此操作
            if (uploadContxt.suspend || uploadContxt.exit)
                return;
            // 触发完成
            if (config && onUploadProgress) {
                UploadSlice.onUploadProgress(undefined, uploadConfig, uploadContxt, onUploadProgress);
    ...
    upload(requestConfig, option) {
        let promiseExtend = new PromiseExtend((resolve, reject) => {
            ...
            if (config) {
                return this.triggerUpload(config, requestConfig, option);
            }
            else {
                promiseExtend = null;
                config = null;
    ...
    // 合并上传进度控制
    static mergeUploadProgress(progress, uploadConfig, onUploadProgress, success) {
        let speed = 0;
        if (progress) {
    ...
    // 请求 merge
    getRequestObject(fileOption, response, uploadConfig) {
    ...
    // 执行队列
    queue({ uploadContxt, fileOption, config, uploadConfig, callback }) {
        // 如果处于暂停状态,暂停执行
        if (uploadContxt.suspend || uploadContxt.end)
            return;
        let index = uploadContxt.getNextSlice();
        if (index === undefined)
            return;
        // 创建使用配置
        let resultUploadConfig = {
            ...uploadConfig,
        ...
        let mergeRequestData = { rest: false, onUploadProgress: useUploadProgress, cancelToken: uploadContxt.getCancelToekn() };
        if (!UploadSlice.jurisdiction) {
            if (config.requestData.file.supportURL) {
                mergeRequestData.url = config.requestData.file.supportURL;
            }
        }
        return this.request.upload(Object.assign({}, config.requestData, mergeRequestData)).then((response) => {
            // 如果仍然存在分片触发
    ...
    static onUploadProgress(progress, resultUploadConfig, uploadContxt, onUploadProgress) {
        return platforms.platform.setTimeout && platforms.platform.setTimeout(function () {
    ...
    // 替换
    repleace(data = {}, replaceData, resultUploadConfig, noIncludes) {
    ...
    createConfig(fileOption) {
        return {
            ...
            name: fileOption.name || fileOption.file.name,
            hash: ''
        };
    }
    // 获取增加 % 数量
    static getAddSpeedNumber(number, total, fileOption) {
        let speed = 100 - (fileOption.analysis || 0) - (fileOption.mergeAnalysis || 0);
        speed = speed < 0 ? 0 : speed;
        return total / speed * number;
    }
    // 获取唯一值
    unique(fileOption, config, requestConfig, uploadContxt) {
        return new Promise((relove, reject) => {
            ...
            if (fileOption.unique) {
                ...
                if (UploadSlice.jurisdiction) {
                    let fileReader = new platforms.platform.FileReader();
                    // @ts-ignore
    ...
    distribution;
    slice(fileOption, index, config) {
        if (UploadSlice.jurisdiction) {
            let start = index * fileOption.splitSize, end = ((start + fileOption.splitSize) >= config.size) ? config.size : start + fileOption.splitSize;
