...
export default class PromiseExtend<T,D = unknown> extends Promise<T> implements InterFacePromiseExtend<T,D> {
    ...
    static all(data:Array<PromiseExtend<any>>){
        return new PromiseExtend(function(relosve,reject){
            let watchContxt:Record<string, any> = {
                length: data.length,
    ...
    constructor(executor:PromiseExtendCallback<T,D>){
        ...
        if(!executor) {
            platforms.platform.error('no trigger Function,this arg is',executor,'but need typeof is Function');
        } else {
            this.triggerTimeCallback = platforms.platform.setTimeout(()=>{
                executor(
    ...
    // 失败的事件收集
    private rejects:Array<Function> = [];

    // 完成的事件收集
    private finallys:Array<Function> = [];
    ...
    // 完成
        ...
        // 清除计数器操作
        platforms.platform.clearTimeout && platforms.platform.clearTimeout(this.triggerTimeCallback);
        // 清空所有回调
